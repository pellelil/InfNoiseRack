...
### Trigger-outputs
By default, trigger outputs in an Infinite-Noise module uses a **1 ms high phase (default 10V) followed by a forced 1 ms low phase (default 0V)**. That gap lets downstream modules see both the rising and falling edge of the trigger, even if they do not check every sample. At 48 kHz, 1 ms is 48 samples (about 44 samples at 44.1 kHz). The forced low phase also stops consecutive triggers from merging into one continuous pulse, so with the default length only one trigger can fire in a 2 ms window (**at most 500 triggers per second**). 
...
Choosing a low cycle count you will be able to generate more than 500 trigger per second (if needed). Likewise you can instead choose a high ms count, where output ports that can otherwise only generate triggers, can be used as "fixed length gates" (longer than 1 ms) - directly - without needing another utility module *(e.g. [ADR Envelope](Envelope.md#adr-envelope) which thanks to its delay-time can generate a fixed length "gate-output" lasting up to 10 seconds - from a single trigger input - see its tips)*.

**Choosing a longer length can also be used to reduce the number of triggers that be generated** by a trigger port. Lets say you have a signal path (one or multiple modules) which outputs a chaotic string of triggers, but perhaps it sometimes generates multiple triggers very close to each other. One way to elliminate these "close triggers" could be to make the low-length longer (e.g. 200 ms). In this aspect all generated triggers would still have a 1 ms high-phase. However the (forced) 200 ms low-phase, ensures when a trigger have fired, another cannot be fired till after this 200 ms low-phase.
...
+ [ADSDR Envelope](Envelope.md#adsdr-envelope): Attack-Decay-Sustain-Delay-Release Envelope with attack/decay/release time and shape, and levels for Attack, Sustain and Release. Times can be affected by Rate Chaos (set via context-menu).
+ [Envelope Phase Expander](Envelope.md#envelope-phase-expander): Expander for ADR/ADSDR Envelope with gate outputs for each phase (Attack, Decay, Sustain, Delay, Release and Idle) plus gate outputs for envelope movement (Rise, Steady and Fall).
