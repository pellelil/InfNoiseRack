...
### Trigger-outputs
By default, trigger outputs use a **1 ms high phase (default 10V) followed by a 1 ms low phase (default 0V)**. That gap lets downstream modules see both the rising and falling edge, even if they do not check every sample. At 48 kHz, 1 ms is 48 samples (about 44 samples at 44.1 kHz). The forced low phase also stops consecutive triggers from merging into one continuous pulse, so with the default length only one trigger can fire in a 2 ms window (**at most 500 triggers per second**). 

However modules that output triggers have a **Trigger length** item in the Triggers context menu. You can keep the default 1 ms, pick a longer time (2, 5, 10, 50, 100, 200, 500 or 1000 ms), or pick a length in engine samples (1, 2, 4, 8, 16, 32, 64, 128 or 256 cycles). On- and off-phases always use the same length. Millisecond settings follow the engine sample rate; cycle settings are a fixed number of samples regardless of rate. 
...
**Warning**: Do not pick a trigger length that is too short: some modules do not look for (or process) triggers every cycle, so a pulse that is high for only a few cycles can be missed. Also be aware of **process quality** on both the receiving and the generating module. Infinite-Noise lets you choose how often a module runs. For example, if a trigger is only high for 4 cycles, but the receiving module only checks for triggers every 64th cycle (**Low**), it can miss many triggers. Likewise, if you set the generating module’s process quality so it only runs every 64th cycle, it cannot produce a high or low phase shorter than 64 cycles—even if Trigger length is set shorter than that. 
