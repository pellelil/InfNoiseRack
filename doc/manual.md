...
### Trigger-outputs
By default, trigger outputs use a **1 ms high phase (default 10V) followed by a forced 1 ms low phase (default 0V)**. That gap lets downstream modules see both the rising and falling edge, even if they do not check every sample. At 48 kHz, 1 ms is 48 samples (about 44 samples at 44.1 kHz). The forced low phase also stops consecutive triggers from merging into one continuous pulse, so with the default length only one trigger can fire in a 2 ms window (**at most 500 triggers per second**). 
