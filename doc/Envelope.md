...
While a traditional ADSR envelope only needs you to specify the Sustain level, the ADSDR Envelope also lets you specify A.level and R.level (all 3 levels can be set in the range from -10 to +10). By default **A.level** is set as 10V, **Sustain** as 5V and **R.level** as 0V. The illustration of the ADSDR Envelope above illustrates **a traditional ADSR Envelolpe** *(with an optional Delay)*. However the fact that you beside Sustain- also is able to manually specify A.level and R.level, you can generate many different envelopes (both **unipolar** and **bipolar**). E.g. for an enveloped controlled panning, you can set R.level to 0V (center of a bipolar range), set A.level to -5 and Sustain to +5. When the Envenlope is triggered, it will first transition from 0 (R.level) to -5 (A.level). Once A.level is reached the decay phase will transition from -5 (A.level) to +5 (Sustain), and finally from +5 (Sustain) back to 0 (R.level) during the release phase of the envelope. *In  this configuration I would probably connect BOS (Begin Of Sustain) to DlR.trig, to ensure the release will be automatically triggered. Hence you only need to trigger ADc.trig from an external source to "initiate a wobble" (from center, then left/right, and back to center).*
