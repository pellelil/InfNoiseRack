...
The **time knobs dial in a rate**, not a fixed duration, unless you let the phase run its full span without interrupting it. Example: A.level = 10V, R.level = 0V, and both A.time and R.time set to 1 second → rise and fall are "10V per 1 second". If Attack is interrupted at 6V, Release only has to travel from 6V to 0V, so it lasts 0.6 seconds. If a new Attack starts from 3V during Release, Attack only has to travel from 3V to 10V (0.7 seconds). **Attack retrig** is the exception: Attack then always begins from R.level, so with those defaults it always lasts the full second.
...
The ADR Envelope is a three-phase **Attack/Delay/Release envelope generator** (or simply a two-phase Attack/Release envelope when the delay time is 0). It ramps the Envelope output between an attack end-level (**A.level**) and a release end-level (**R.level**), with independent time and shape for each phase. Defaults are A.level = 10V and R.level = 0V, which covers the usual unipolar envelope case, but you can set any levels you might need. At the bottom of the module you find 6 outputs: the first 4 fire a trigger when the attack/release phases begin/end, plus **Env** and **!Env**. *Some general info regarding the Envelopes are listed at the top.*
...
The 4 time knobs let you specify a time between 0 and 10 seconds. Typical A.level and R.level will be your extremes, and their default span is 10V (default: A.level=10V, R.level=0V). So if you set A.time to 1 second the attack rate will be 10V per 1 second. Decay only has to travel from A.level to Sustain, so with the defaults (10V → 5V) a Dc.time of 1 second it will have a rate of 5V per 1 second. Likewise Release only has to travel from Sustain to R.level, so with the defaults (5V → 0V) a R.time of 1 second it will also have a rate of 5V per 1 second.

*So with the default levels (A.level=10V, Sustain=5V and R.level=0V), if you want the slope of the envenlop to be similar for Attack, Decay and Release, you would need to dial in an A.time that is double of Dc.time and R.time, since the attack have to transition 10V (0V to 10V), whereas decay and release only have to transition 5V (decay from 10V to 5V, and release from 5V to 0V). If you instead set the same time for both A.Time, Dc.time and R.time each of these phases, they will have the same duration (if allowed to run uninterrupted), however the slope of attack will be more steep than the slope of decay and release.*
