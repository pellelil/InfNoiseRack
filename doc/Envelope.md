...
For the attack/release phases you both find a begin-trigger output (e.g. **BOA=Begin Of Attack**) and an end-trigger output (e.g. **EOA=End Of Attack**). These will fire whenever a phase begins and ends. If Attack retrig is enabled, the BOA trigger will fire each time you trigger the A.trig input, whether that phase is currently active or not. If you have dialed in a non-zero delay, the **BOR (Begin Of Release)** won't fire till the delay has finished. The end-trigger outputs ("EOA" and "EOR") will only fire if the phase runs to the end. E.g. if you input a trigger into A.Trig before the previous release finishes, EOR will not fire. 
...
While a traditional ADSR envelope only needs you to specify the Sustain level, the ADSDR Envelope also lets you specify A.level and R.level (all 3 levels can be set in the range from -10 to +10). By default **A.level** is set as 10V, **Sustain** as 5V and **R.level** as 0V. 
...
Among the outputs you'll find the same 4 triggers as the ADR Envelope: **BOA** (Begin Of Attack), **EOA** (End Of Attack), **BOR** (Begin Of Release) and **EOR** (End Of Release). The ADSDR Envelope also has trigger outputs for **BOS (Begin of Sustain)** and **EOS** (End Of Sustain), however you won't see trigger outputs for "Begin of Decay" or "End Of Decay". The EOA is basically the same as "Begin of Decay", and BOS is basically the same as "End Of Decay". Like the ADR Envelope you don't find named trigger-outputs for begin/end of Delay, but basically (if delay time is non-zero) EOS = Begin of Delay, and BOR = End of Delay.
...
**TIP**: The illustration of the ADSDR Envelope further above illustrates **a traditional ADSR Envelope** *(with an optional Delay)*. However the fact that you beside **Sustain-** also is able to manually specify **A.level** and **R.level**, you can generate many different envelopes (both **unipolar** and **bipolar**). E.g. for an enveloped controlled "panning-wobble", you can set R.level to 0V (center of a bipolar range), set A.level to -5 and Sustain to +5. When the Envenlope is triggered, it will first transition from 0 (R.level) to -5 (A.level). Once A.level is reached the decay phase will transition from -5 (A.level) to +5 (Sustain), and finally from +5 (Sustain) back to 0 (R.level) during the release phase of the envelope. In  this configuration I would connect BOS (Begin Of Sustain) to DlR.trig, to ensure the release will be automatically triggered. Hence you only need to trigger ADc.trig from an external source to "initiate a wobble" (from center, then left/right, and back to center).

![Envelope controlled wobble](img/EnvelopeWobble.png)

*Perhaps not the best example - In this particular case, you could acomplish a simiar output using the triangle waveform from a [LFO1](LFO.md#lfo1), where you provide a trigger-input to its Sync-input in n-shot mode. However that doesn't "take away from" the ADSDR Envelope being able to generate many different outputs due to both is different times, levels and shapes.*

**TIP**: For a random jagged signal (envelope basically switching between Attack and Release) you can use the A- and B-outputs of a [Bernoulli Switch](Switch.md#bernoulli-switch) as inputs to ADc.trig and DlR.trig. Using the Clock knob of the Bernoully Switch you can set the rate at which Adc.Trig and DlR.trig can trigger a change. If you center the Probabillity switch of the Bernoully knob, and input a slow moving bipolar waveform (e.g. Sine) into it's Probabillity CV-input you can change the probablility of the Bernoulli Switch either triggering an Attack or a Release *(increase the chance the output will occupy the full range)*.
...
**Auto** (default): connect when exactly one neighbor is an ADR/ADSDR Envelope. If both sides are envelopes, both lights go red and no gates are driven until you choose Left or Right. **Left** / **Right**: always use that side; red on that light if no envelope is there, otherwise green. *Basically "what you want to see" is a sigle green light, indicating if it is connected to an Envelope module on its left- or right side.*

The first six outputs are **mutually exclusive phase gates** (exactly one high at a time). A phase that is skipped (time=0) never becomes active, so its gate stays low. **Decay** stays low for ADR Envelope (that module has no Decay stage). **Idle** is high while the envelope holds at R.level after Release (or at init).

Likewise the last three outputs are also **mutually exclusive movement gates** (exactly one high at a time). In a typical envelope attack is always a rise, whereas decay and release are typical falls. However as you can freely set A.level, Sustain and R.level for an ADSDR Envelope, this is not set in stone (e.g. the Attack can be fall, and the Decay can be a rise. Rise and Fall switch on the first moving sample. **Stdy** waits until the envelope has been unchanged for 3 consecutive samples (avoids a 1-sample spike at phase seams, e.g. when BOS is patched to DlR.trig). 

![Screenshot of Envelope Phase Expander](module/EnvelopePhaseExpander.png)

**TIP**: If you need to check multiple phases at the same time, you might want to use this module with a [Logic Comparator-6x2](Compare.md#logic-comparator-6x2). E.g. pass both the Attack- and Decay outputs into the same section of the Logic Comparator-6x2 and set the logic 3-way switch to **OR**, in order to generate a high gate, when the envelop is either in the Attack or Decay phase. *Alternative simply route both the Attack and Decay-gates into the same input. When multiple cables are connected to the same input, all signals will simply be added. However as Attack and Decay are never high at the same time, the receiving input will by default either see 10V if one of these are high, or 0V if neither are high.*
