...
This 2HP module checks whether the CV input signal falls within a specified range. The range is defined using the **Min** and **Max** knobs and their corresponding CV inputs. The **Min** and **Max** controls each include a button to determine whether the Min/Max values are **inclusive** (i.e., part of the range). At the bottom of the module, you’ll find four outputs, which by default all produce gates:
