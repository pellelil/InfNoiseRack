...
struct SLFO4stModule : InfNoiseModule {
    enum ParamId {
        FREQKNOB1_PARAM,
        FREQKNOB2_PARAM,
        FREQKNOB3_PARAM,
    ...
    SLFO4stModule() : InfNoiseModule() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            configOutput(WAVE1_1ST_OUTPUT + i, string::f("LFO-%d Square-waveform", i + 1));
            configOutput(WAVE2_1ST_OUTPUT + i, string::f("LFO-%d Triangle-waveform", i + 1));
    ...
    void onReset(const ResetEvent& e) override {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            freq[i] = 2.f;
            phase[i] = 0.f;
            phaseStep[i] = sampleTime; // set/updated in processParams
            syncSign[i] = 1.f;
            chaosAmount[i] = 0.f;
            chaosFactor[i] = 1.f;
...
struct SLFO4stModuleWidget : InfNoiseModuleWidget {
    SLFO4stModuleWidget(SLFO4stModule *module) {
        initializeWidget(module, "res/SLFO4st");
        ...
        for (int i = 0; i < 4; i++) {
            // Freq-knobs
            addParam(createParamCentered<RoundLargeBlackKnob>(Vec(freqKnobCol, freqKnobRow), module, SLFO4stModule::FREQKNOB1_PARAM + i));
            freqKnobRow += freqKnobSpacing;

            // Output-range toggles
            addParam(createParamCentered<CKSSThree>(Vec(rangeToggleCol, rangeToggleRow), module, SLFO4stModule::RANGE1_TOGGLE_PARAM + i));
            rangeToggleRow += freqKnobSpacing;

            // Waveform outputs
			addOutput(createOutputCentered<ThemedPJ301MPort>(Vec(wave1OutputCol, waveOutputRow), module, SLFO4stModule::WAVE1_1ST_OUTPUT + i));
			addOutput(createOutputCentered<ThemedPJ301MPort>(Vec(wave2OutputCol, waveOutputRow), module, SLFO4stModule::WAVE2_1ST_OUTPUT + i));
            addChild(createLightCentered<TinyLight<RedLight>>(Vec(29.578f, waveOutputRow + 10.421f), module, SLFO4stModule::INV1_LIGHT + i));
            waveOutputRow += waveOutputSpacing;

            // Freq-lights
            addChild(createLightCentered<SmallLight<GreenLight>>(Vec(freqLightCol, freqLightRow), module, SLFO4stModule::FREQ1_LIGHT + i));
            freqLightRow += freqKnobSpacing;

            // Sync-lights
            if (i < 3) {  // Only 3 sync-lights
                int syncLightIdx = i * 2;
                addChild(createLightCentered<TinyLight<GreenRedLight>>(Vec(syncLightCol, syncLightRow), module, SLFO4stModule::SYNC2_LIGHT + syncLightIdx));
    ...
    void appendContextMenu(Menu* menu) override {
        ...
        SLFO4stModule* module = dynamic_cast<SLFO4stModule*>(this->module);
        assert(module);
...
Model *modelSLFO4st = createModel<SLFO4stModule, SLFO4stModuleWidget>("SLFO4st");
