...
struct FlipFlopModule : InfNoiseModule {
    ...
    bool qHigh = false; // True if Q is high, else Q is low
    bool disabled = false;
    bool haveOutputs = false;
    bool undefOpr = false;  // True if "undefined operation" detected in SR-mode
    ...
    infNoiseOutTrigger qChangedTrigger = infNoiseOutTrigger();
    ...
    void dataFromJson(json_t* rootJ) override {
        ...
        qHigh = getJsonInt(rootJ, "qHigh", 0) == 1;
    ...
    void dataToJson(json_t* rootJ) override {
        ...
        json_object_set_new(rootJ, "qHigh", json_integer(qHigh ? 1 : 0));
    ...
    void applyLoadedOutputs() {
        ...
        if (outputs[Q_OUTPUT].isConnected()) {
            outputs[Q_OUTPUT].setVoltage(qHigh
                ? voltValues[gateOutHigh.act]
                : voltValues[gateOutLow.act]);
        }
        if (outputs[INVQ_OUTPUT].isConnected()) {
            bool qInv = undefOpr
                ? (undefOprOutput.act == undefOprOutputType::uoo_low_high ||
                    undefOprOutput.act == undefOprOutputType::uoo_high_high)
                : !qHigh;
        ...
        if (outputs[CHQ_OUTPUT].isConnected()) {
            outputs[CHQ_OUTPUT].setVoltage(voltValues[trigOutLow.act]);
        }
    }
    ...
    void processParams(const ProcessArgs& args) {
        ...
        if (mode != lastMode) {
            ...
            if (mode < 0.5f)  // T-mode
            {
                inputInfos[TD_INPUT]->name = monoPortPrefix() + tdName[0];
                lights[T_LIGHT].setBrightness(1.f);
                lights[D_LIGHT].setBrightness(0.f);
            }
            else // D-mode
            {
                inputInfos[TD_INPUT]->name = monoPortPrefix() + tdName[1];
    ...
    void process(const ProcessArgs& args) override {
        ...
        if (doProcess && haveOutputs) {
            ...
            if (!disabled) {
                float prevQHigh = qHigh;

                // Set and reset inputs (takes priority over clock and data inputs)
                float setInput = params[SET_PARAM].getValue() > 0.5f
                    ? 10.f
                    : inputs[SET_INPUT].isConnected() 
                        ? inputs[SET_INPUT].getVoltage() 
                        : 0.f;
                bool setGate = setInput >= trueDetectValues[gateDetHigh.act];

                float resetInput = params[RESET_PARAM].getValue() > 0.5f
                    ? 10.f
                    : inputs[RESET_INPUT].isConnected() 
                        ? inputs[RESET_INPUT].getVoltage() 
                        : 0.f;
                bool resetGate = resetInput >= trueDetectValues[gateDetHigh.act];
                ...
                if (setGate && resetGate) // Undefined operation
                {
                    ...
                    {
                        qHigh = (undefOprOutput.req == undefOprOutputType::uoo_high_low ||
                ...
                else if (resetGate) // Reset only
                {
                    ...
                    float tdInput = inputs[TD_INPUT].isConnected() 
                        ? inputs[TD_INPUT].getVoltage() 
                        : 0.f;
                    float clockInput = inputs[CLOCK_INPUT].isConnected() 
                        ? inputs[CLOCK_INPUT].getVoltage() 
                        : tdInput; // Normalize to TD-input
                    if (clockTrigger.process(clockInput, 
                        trueDetectValues[trigDetLow.act], trueDetectValues[trigDetHigh.act])) {

                        if (params[MODE_PARAM].getValue() < 0.5f)  // T-mode
                        {
                        ...
                        {
                            qHigh = tdInput >= trueDetectValues[gateDetHigh.act];
                        }
                    }
                }
                ...
                if (updateOutput)
                {
                    ...
                    bool qInv = (undefOpr)
                        ? (undefOprOutput.req == undefOprOutputType::uoo_low_high ||
                            undefOprOutput.req == undefOprOutputType::uoo_high_high)
                        : !qHigh;
                    outputs[INVQ_OUTPUT].setVoltage(qInv ? voltValues[gateOutHigh.act] : voltValues[gateOutLow.act]);
                }

                // Set Q changed output (trigger)
                if (!qChangedTrigger.process(procSampleTime) && prevQHigh != qHigh) {
                    qChangedTrigger.trigger();
                }
                outputs[CHQ_OUTPUT].setVoltage(qChangedTrigger.isHigh()
...
struct FlipFlopModuleWidget : InfNoiseModuleWidget {
    FlipFlopModuleWidget(FlipFlopModule *module) {
        ...
        addInput(createInputCentered<ThemedPJ301MPort>(Vec(cntrClm, 79.801f), module, FlipFlopModule::TD_INPUT));
        
        // Clock/Reset input
        addInput(createInputCentered<ThemedPJ301MPort>(Vec(cntrClm, 116.844f), module, FlipFlopModule::CLOCK_INPUT));
        ...
        addOutput(createOutputCentered<ThemedPJ301MPort>(Vec(cntrClm, 262.591f), module, FlipFlopModule::Q_OUTPUT));
        addOutput(createOutputCentered<ThemedPJ301MPort>(Vec(cntrClm, 298.189f), module, FlipFlopModule::INVQ_OUTPUT));
        addOutput(createOutputCentered<ThemedPJ301MPort>(Vec(cntrClm, 332.694f), module, FlipFlopModule::CHQ_OUTPUT));
    }
